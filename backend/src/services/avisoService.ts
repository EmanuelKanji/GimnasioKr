...
const crearAvisoVencimiento = async (alumno: any) => {
  const hoy = new Date();
...
export const enviarAvisosAutomaticos = async () => {
  try {
...
export const verificarPlanesVencidos = async () => {
  try {
    console.log('🔍 Verificando planes vencidos...');
    
    const hoy = new Date();
    const alumnosVencidos = await Alumno.find({
