...
describe('Validación de esquemas', () => {
    describe('Esquema de login', () => {
        test('debe validar login correcto', () => {
            ...
            const { error } = validation_1.schemas.login.validate(validData);
        ...
        test('debe rechazar login sin username', () => {
            ...
            const { error } = validation_1.schemas.login.validate(invalidData);
    ...
    describe('Esquema de crear alumno', () => {
        test('debe validar alumno correcto', () => {
            ...
            const { error } = validation_1.schemas.createAlumno.validate(validData);
            expect(error).toBeUndefined();
        });
        test('debe usar valor por defecto para limiteClases', () => {
            ...
            const { value, error } = validation_1.schemas.createAlumno.validate(dataWithoutLimit);
            expect(error).toBeUndefined();
        ...
        test('debe rechazar limiteClases inválido', () => {
            ...
            const { error } = validation_1.schemas.createAlumno.validate(invalidData);
            expect(error).toBeDefined();
    ...
    describe('Esquema de crear plan', () => {
        test('debe validar plan correcto', () => {
            ...
            const { error } = validation_1.schemas.createPlan.validate(validData);
            expect(error).toBeUndefined();
        });
        test('debe rechazar duración inválida', () => {
            ...
            const { error } = validation_1.schemas.createPlan.validate(invalidData);
            expect(error).toBeDefined();
